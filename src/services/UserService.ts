...
export class UserService {
  ...
  constructor(repository = db.getRepository(User)) {
  ...
  createUser = async ({
    ...
    cod_cidade,
    nivel_acesso,
  }: UsuarioRequest): Promise<User | Error> => {
    try {
      const itemDb = await this.repo.findOne({
        where: { login: login, cod_cidade: Number.parseInt(cod_cidade) },
      });
      if (itemDb) {
  ...
  updateUser = async ({
    ...
    category.cod_cidade = Number.parseInt(cod_cidade)
      ? Number.parseInt(cod_cidade)
      : category.cod_cidade;
  ...
  validateUser = async ({ login, senha }: LoginRequest) => {
    ...
    if (category) {
      if (category.status == 0) {
        return new Error("Usuario inativo!");
      }
      ...
      const dataCidade = await db
        .getRepository(Cidades)
        .findOne({ where: { id: category.cod_cidade } });
      return {
        token,
        nomeCidade: dataCidade.descricao,
        long: dataCidade.long,
        lat: dataCidade.lat,
      };
    } else {
